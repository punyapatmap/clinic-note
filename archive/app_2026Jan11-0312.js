...
// Which external HTML file to load for each print mode
...
async function loadPrintTemplate(mode) {
  ...
  try {
    ...
    // find the first element that starts with id="printArea-..."
    const area = printHost.querySelector('[id^="printArea-"]');
    currentPrintAreaId = area?.id || currentPrintAreaId;

    // IMPORTANT: after replacing HTML, you must re-bind preview fields
    bindPreviewFields();

    // and re-sync mode-specific print blocks (dx/med/mc dx)
    syncAllPrint(mode);
  } catch (err) {
    console.warn("Could not load print template:", url, err);
    printHost.innerHTML = `<div class="note noteA4" style="padding:16mm">Template load error: ${escapeHtml(url)}</div>`;
...
function createDxRow({ text = "", type = "Primary" } = {}) {
  const tr = document.createElement("tr");
  tr.dataset.icd10 = "";   // store code here
...
function attachDxAutocomplete(input) {
  ...
  // show English only in OPD Dx list
  input.value = d.en || "";

  // store ICD10 on the row (so MC can lookup Thai later)
  const tr = input.closest("tr");
  if (tr) tr.dataset.icd10 = d.icd10 || d.id || "";
...
function syncDxToPrint() {
  const dxPrintList = $("dxPrintList"); // re-find every time
...
function syncMedToPrint() {
  const medPrintTbody = $("medPrintTbody"); // re-find every time
...
function initVisitDtDefault(force = false) {
  ...
  // Only set if empty, unless force=true
...
async function loadPrintTemplateInto(areaId, path) {
  const el = $(areaId);
  if (!el) return;

  const res = await fetch(path, { cache: "no-store" });
  if (!res.ok) throw new Error(`Load failed ${path}: HTTP ${res.status}`);
